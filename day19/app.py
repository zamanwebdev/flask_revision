...
def dashboard():
    ...
    if session.get('role') == 'admin':
        query = "SELECT tasks.id, tasks.title, tasks.status, users.username FROM tasks JOIN users ON tasks.user_id=users.id"
        params = ()
        if filter_status:
...
def edit_task(id):
    ...
    if request.method == 'POST':
        ...
        cur.execute("UPDATE tasks SET title=? WHERE id=? AND user_id=?",
